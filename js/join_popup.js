...
(function () {
  ...
  ready(function () {
    ...
    // Homepage detection (works for your local URL /VIS2KNOW_Lab/ and GitHub Pages)
    var path = window.location.pathname.replace(/\/+$/, "");
    var base = "/VIS2KNOW_Lab";  // <-- your baseurl

    var isHome = (path === "" || path === "/" || path === base || path === base + "/index.html");
    if (!isHome) return;

    // Requires Bootstrap + jQuery
    if (typeof window.jQuery === "undefined") {
      console.log("Popup: jQuery missing");
      return;
    }
